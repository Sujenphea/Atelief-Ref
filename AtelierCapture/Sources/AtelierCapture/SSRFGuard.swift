// AtelierCapture — SSRF guard for every fetch of attacker-influenceable content
// (001 · C2b; moved here from AtelierIngestion in 457).
//
// The Mac fetches ATTACKER-INFLUENCEABLE content — a pasted page's HTML and the
// og:image URL that HTML names (001 · O3, approved 2026-07-13). That bends the "app
// never downloads" invariant, so every such fetch is walled: only an http(s) URL whose
// host resolves EXCLUSIVELY to public IP addresses may be fetched. This blocks the
// classic SSRF targets — the cloud metadata endpoint (169.254.169.254), loopback
// (127.0.0.1 / ::1), RFC1918 / ULA / link-local ranges — before any bytes move.
//
// **Why it lives in AtelierCapture.** The share extension makes the identical fetch —
// tier 2 (092 · S4b) downloads the media URL a page's DOM named, which is a URL the
// page chose — and had no wall at all (098 · finding 2). The extension links this
// package and cannot link the ingest pipeline, so the guard moved to the one package
// both fetchers reach; `AtelierIngestion` typealiases it the way it does
// `ContentHasher`, so every Mac call site keeps the name it already uses and there is
// exactly one guard in the program. Foundation + Network only, which is what makes it
// linkable from a memory-capped extension. The extension's fetch adopts it in 098 · P5;
// nothing about the Mac's use changed in the move.
//
