...
| **G14** | **ToS / "downloader" policy risk.** Bulk sweep + video-resolution paths are exactly what Web Store review and X/Pinterest ToS target; the human-pacing jitter can read as evasion. Needs a distribution-channel decision (unlisted vs public vs unpacked-only) + a **privacy policy**. The **`instagram.com` host permission** added for the IG saved-posts sweep (002 · B2) widens this surface further — Meta ToS + the account-challenge risk the sweep's warning UI (002 · B4) must disclose. | `config.js`, `twitter-video.js`, `bulk-pinterest.js`, `manifest.json` |
