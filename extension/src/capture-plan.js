 ...
 *
 * @returns `{ kind, videoUrl, urlCandidates, content, blocked, reason }`
 ...
 */
export function planCapture(provenance, { mp4Url = null, content = null, isAllowedHost = null } = {}) {
  ...
  if (mp4Url) {
    return { kind: CAPTURE_KIND.video, videoUrl: mp4Url, urlCandidates, content, blocked, reason: null };
  }
  if (urlCandidates.length) {
    return {
      kind: content ? CAPTURE_KIND.content : CAPTURE_KIND.image,
      videoUrl: null, urlCandidates, content, blocked, reason: null,
    };
  }
  ...
  if (content) {
    return { kind: CAPTURE_KIND.content, videoUrl: null, urlCandidates: [], content, blocked, reason: null };
  }
  return {
    kind: CAPTURE_KIND.none, videoUrl: null, urlCandidates: [], content: null, blocked,
    reason: blocked.length ? "blocked-host" : "no-media",
  };
}
