...
- **[021]** — `run-local.command` builds to `build/local-release` and launches
...
- **[022]** — ⌫ means a *different verb* on each surface: destroy in the grid,
...
- **[025]** — the sidebar chevron is an `NSImageView` ("indicator only — the row
...
- **[027]** — the grid's right-click Move to ▸ uses `moveTargets` (direct
  ...
  preventing.
- **[028]** — `CanvasArrange.tidy` has **no width bound**, and `tidyRows` clusters
...
- `.docs/feature-todo/021-local-install-lane.md` (new)
- `.docs/feature-todo/022-two-tier-delete.md` (new)
- `.docs/feature-todo/023-archive-and-second-library.md` (new)
- `.docs/feature-todo/024-keyboard-map-and-shortcuts-page.md` (new)
- `.docs/feature-todo/025-sidebar-row-interaction.md` (new)
- `.docs/feature-todo/026-item-detail-gaps.md` (new)
- `.docs/feature-todo/027-grid-destinations-and-carousel-scope.md` (new)
- `.docs/feature-todo/028-spaces-tidy-wraps.md` (new)
...
1. **[027] G1 before [022] D2.** The lead-widening bug is one line; [022] is about
   to wire a second consumer (`removeSelectedFromFolder`) onto the same broken
   property.
2. **[022] and [027] G1 in one window.** Together they change ⌫ on a carousel tile
   from "destroy one image" to "remove all four from this collection" — two
   surprises unless described as one change.
3. **[026] §A and [027] §G2 share an extraction.** Both need the destination list
